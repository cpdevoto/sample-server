...
import static testutils.rules.dockerdb.RuleUtils.*;
...
class DockerDatabaseStatement extends Statement {
  ...
  private static final String DOCKER_DEVELOP_REPO = "shrinedevelopment-docker-develop.jfrog.io";
  private static final String DOCKER_STABLE_REPO = "shrinedevelopment-docker-stable.jfrog.io";
  ...
  private void startDockerDatabase() throws DockerException, InterruptedException, IOException {
    ...
    try {
      ...
      log.info("Did not remove image " + image + " because it could not found in the local Docker repository");
    }
  ...
  private ContainerConfig containerConfig() {
    ...
    portBindings.put(databasePort, ImmutableList.of(PortBinding.of(databaseHost, databasePort)));
  ...
  private Optional<String> getNonDevelopmentImageName() {
    // If this is a non-development build, a docker-dependencies.lock file will exist, and it will contain an entry 
    ...
    try (InputStream in = new FileInputStream(f)) {
      ...
      if (image == null) {
        throw new DockerDependencyLockReadException("Expected a 'postgres' entry in the docker-dependencies.lock file");
      }
